...
import re
import datetime
from bank_2.models import *
from django.shortcuts import render_to_response,redirect
from bank_2.forms import *
import importlib
import hashlib
from bank_2 import forms
from bank_2 import methods_defn
...
def sign_in_post(request):                                  #POST method handler for sign in page
    if  "sign_button" in  request.POST:                     #if user is signing in
        ...
        if user:
            ...
            return render_to_response("Sign_in.html", {"bank":"XYZ Bank","error": error})

    elif "register_button" in request.POST:                 #if registration request is obtained in sign in page
        return HttpResponseRedirect('/project/banks/bank_2/register/')
...
def sign_in_get(request):                                   #GET method handler for sign in page
    return render_to_response("Sign_in.html", {"bank":"XYZ Bank","error": 0})
...
def register_post(request):
    if "sign_button" in request.POST:
        ...
        if form_data:
            ...
            return render_to_response("Register.html ", {"bank":"XYZ Bank","form": form, "empty_errors": errors[0],
...
def register_get(request):
    ...
    return render_to_response("Register.html", {"bank":"XYZ Bank","form": form,})
...
def welcomepage(request, user):
    ...
    page_redirect = '/project/banks/bank_2/' + user.username + '/'
...
#        request.session['balance'] = ""
    ...
    return render_to_response("Welcome.html", {"bank":"XYZ Bank","user": username, "error":error, "bal":balance})
...
def transfer_request(request):
    ...
    to_acc_no = "eWallet"
    #    transfer_to_ew(ew_user,amount,bank)
...
def admin_page(request):
        ...
        if username == "admin" and password == "admin":
            ...
            if"showincome" in request.POST:

                    incoming_trans = Incoming_pends.objects.filter(status=1)
                    return render_to_response("admin.html", {"bank":"XYZ Bank",'incoming': incoming_trans,
                                                             'view_in': 1, 'view_out': 0})

            if"showoutgo" in request.POST:

                outgoing_trans = Incoming_pends.objects.filter(status=1)
                return render_to_response("admin.html", {"bank":"XYZ Bank",'outgoing': outgoing_trans,
                                                         'view_in': 1, 'view_out': 0})
            ...
            if "incoming" in request.POST:
                ...
                request.session['username'] = ''
                request.session['password'] = ''
                return render_to_response("admin_signin.html", {"bank":"XYZ Bank","error": error})
            return render_to_response("admin.html",{"bank":"XYZ Bank",})
        if request.method == 'POST':
...
def deposit(request):
    ...
    return render_to_response("admin.html", {"bank":"XYZ Bank","success": success, "valuerror": valuerror})
...
def outgoing(request):
    ...
    if request.session['username'] == 'admin' and request.session['password'] == 'admin':
        ...
        return render_to_response("admin.html", {"bank":"XYZ Bank",'outgoing': outgoing_trans, "view_in": 0, 'view_out': 1})
    return HttpResponseRedirect('/project/banks/bank_2/admin')
...
def incoming(request):
    ...
    if request.session['username'] == 'admin' and request.session['password'] == 'admin':
        if  'incoming' in request.POST:
            settle_incoming()
        incoming_trans = Incoming_pends.objects.filter(status=1)
        return render_to_response("admin.html", {"bank":"XYZ Bank",'incoming': incoming_trans, 'view_in': 1, 'view_out': 0})
